        ...
        override fun onFling(
            ...
            return if (abs(diffX) > abs(diffY)) {
                if (abs(diffX) > SWIPE_THRESHOLD && abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX > 0) {
                        this@MainActivityV2.onSwipeRight(diffX, velocityX)
                    } else {
                        this@MainActivityV2.onSwipeLeft(diffX, velocityX)
                    }
                    true
                } else {
                ...
                if (abs(diffY) > SWIPE_THRESHOLD && abs(velocityY) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffY > 0) {
                        this@MainActivityV2.onSwipeTop(diffY, velocityY)
                    } else {
                        this@MainActivityV2.onSwipeDown(diffY, velocityY)
                    }
                    true
                } else {
                    super.onFling(eventDown, eventMove, velocityX, velocityY)
